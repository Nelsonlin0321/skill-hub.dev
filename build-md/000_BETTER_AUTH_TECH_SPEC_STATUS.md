...
4. `Completed` Create a sign-in page with Better Auth Google client integration.
   Reason: `app/sign-in/page.tsx` and `components/auth/auth-card.tsx` implement the dedicated `/sign-in` experience, pending state, retry-safe error copy, and Google OAuth initiation.
...
- `Completed` Google Cloud Console callback registration.
  Reason: the OAuth client now allows the Better Auth callback URL derived from `BETTER_AUTH_URL`.

- `Completed` Real environment variable provisioning in local and deployed environments.
  Reason: the required secrets and OAuth credentials have been provisioned through the runtime environment.
...
- `Completed` Email/password sign-up and sign-in.
